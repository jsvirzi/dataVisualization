...
int convertLidarPacketToLidarData(LidarDataPacket *lidarDataPacket, LidarData *lidarData) {
  return convertLidarPacketToLidarData(lidarDataPacket, lidarData, -M_PI, M_PI);
}

int convertLidarPacketToLidarData(LidarDataPacket *lidarDataPacket, LidarData *lidarData, double minAzimuth, double maxAzimuth) {
   ...
   * TODO this will fail at phi = 0 (where the cable inserts into the lidar
   * puck)
   */
  ...
  for (iBlock = 0; iBlock < nLidarBlocks; ++iBlock) {
    ...
    if ((minAzimuth <= azimuth0) && (azimuth0 <= maxAzimuth)) {
...
int isBeginningPacket(LidarDataPacket *packet) {
  ...
  //    printf("isBeginningPacket(LidarDataPacket packet=%p): azimuth = %.5f\n",
  //    packet, azimuth);
  double c = cos(azimuth), s = sin(azimuth);
  if ((fabs(c - 1.0) < 0.1) && (fabs(s - 0.0) < 0.1))
    return 1;
  else
    return 0;
}
...
double getAzimuth(LidarDataBlock *block) {
  ...
  double azimuth =
      azimuthData * M_PI / 18000.0 -
      M_PI;  // azimuth is reported in 0.01 degrees. convert to radians
  return azimuth;
}
